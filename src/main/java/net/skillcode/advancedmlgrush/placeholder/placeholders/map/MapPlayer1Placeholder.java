...
@Singleton
public class MapPlayer1Placeholder extends Placeholder {

    private final MapInstanceManager mapInstanceManager;

    @Inject
    public MapPlayer1Placeholder(final @NotNull MapInstanceManager mapInstanceManager) {
    ...
    public String onRequest(final @NotNull Optional<Player> optionalPlayer) {
        ...
        return Optional.ofNullable(mapInstance.getPlayers().get(0)).map(HumanEntity::getName).orElse(getNullValue());
    }
}
